...
runtime = "runc"
if os.environ.get("FORCE_NVIDIA_RUNTIME", False):
    runtime = "nvidia"
...
class SupportMatrixExtractor:
    ...
    def get_from_python(self, lookup: str, key=None):
        ...
        if err != 0:
            ...
            return
        if len(output.decode().split("\n")) == 2:  # ['2.0.9', '']
            self.contdata[key] = output.decode().strip()
        else:
            logger.info(
                "Failed to extract version from python output: %s", output.decode()
            )
